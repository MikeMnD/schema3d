...
This produces a **self-contained HTML file** at `dist/standalone/index.html`
(~2.7 MB) with every script, style, font, the favicon and the bundled schema
inlined, plus a ready-to-send `Schema3D-standalone.zip` right next to it
(containing the same file as `Schema3D.html`). Opening the HTML is running
it — double-click and it loads in the browser straight from `file://`.
Verified to work fully offline: parsing, 3D rendering, labels, filtering,
search and PNG/CSV export are all client-side.

To distribute it, send the `.html` as-is or the zip — both work (some mail
and chat gateways block bare HTML attachments; the zip is the reliable
fallback). If a private schema dump is present at
`client/src/schemas/sample-schemas/cosher.sql` (gitignored), it is baked
into the file — treat the resulting HTML/zip as confidential in that case
and don't commit it.
